...
module.exports = {
	signUp: async (req, res) => {
		try {
			...
			let { email, firstName, lastName, contactNumber, city, zip, dob, gender } = candidate;
			res.json({
				success: true,
				candidate: {
					email,
	...
	login: async (req, res) => {
		try {
			...
			if (!candidate) return res.json({ success: false, msg: 'incorrect credentials' });
	...
	getCurrentUser: async (req, res) => {
		try {
			var candidate = await Candidate.findById(req.user.userId).populate('skills', 'name').select('-password');
	...
	updateProfile: async (req, res) => {
		try {
			...
			var candidate = await Candidate.findByIdAndUpdate(req.user.userId, req.body, {
				new: true,
			}).select('-password');
