...
class Messages extends Component {
	...
	handleSubmit = (e) => {
		...
		var receiverId = this.state.receiverId || this.props.match.params.receiverId;
		if (this.state.text) {
			Axios.post(`/api/v1/employers/chats/${this.props.employer.currentEmployer._id}/messages/${receiverId}`, {
	...
	handleConversation = (id, f, l) => {
		...
		Axios.get(`/api/v1/employers/chats/${this.props.employer.currentEmployer._id}/messages/${id}`)
			.then((res) => {
		...
		return (
			<>
				...
				{this.state.conversations.map((c) => {
					return (
						<Grid sm={6}>
							<Button href={`/employers/dashboard/messages/${c.candidateId._id}`} color="secondary">
												...
												if (a.senderType == 'employer') {
													return (
														<>
															...
															<div class="received">
																<h5 class="hour">{`${new Date(a.createdAt)}`.substr(16, 5)}</h5>
